...
def salvar_lista_csv(lista, N_lista, nome):
    file = os.path.join(raiz,'output', f'lista_{nome}_{N_lista}.csv')
...
def gerar_lista_ordenada(N_lista):
    ...
    salvar_lista_csv(lista, N_lista, "ordenada")

def gerar_lista_inversa(N_lista):
    lista=lista_ordenada[0:N_lista]
    lista.reverse()
    salvar_lista_csv(lista, N_lista, "inversa")

def gerar_lista_aleatoria(N_lista):
    lista = np.array( lista_ordenada[0:N_lista] )
    np.random.shuffle(lista)
    salvar_lista_csv(lista, N_lista, "aleatoria")
...
def gerar_lista_quase_ordenada(N_lista,passo=10):
    ...
    salvar_lista_csv(lista, N_lista, "quase")
